/**
 * This file is based on the ScrollControls of the pmndrs/drei
 * package, just needed to get the scroll direction
 *
 * TODO maker a PR on original repo
 */
    ...
    React.useEffect(() => {
        if (events.connected === el) {
            ...
            const onScroll = () => {
                ...
                if (infinite) {
                    if (!disableScroll) {
                        state.direction = Math.sign(current - previous);
